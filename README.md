...
| **GitHub** | GitHub platform ops (issues, PRs, Actions, releases). Pack = **skill + bound-only official remote GitHub MCP** (server id `github`, auto-seeded, not in the market) with degrade **MCP → [`gh`](https://cli.github.com/) → `git`**. First `delegate_agent` → `github` prepends `[github-access: mcp\|gh\|git\|none]`: MCP when PAT/OAuth configured; else `gh`; else plain `git` (remotes/push/fetch only — Issues/PRs need MCP/`gh`). |
