...
func NewRouter(h *Handler, cfg RouterConfig) *gin.Engine {
	...
	api.GET("/sessions/:id/usage", getSessionUsage(h))
	...
	api.GET("/projects/:id/usage", getProjectUsage(h))
	api.GET("/usage/series", getUsageSeries(h))
	api.GET("/usage/summary", getUsageSummary(h))
	api.GET("/usage/models", listUsageModels(h))
	api.GET("/usage/agents", listUsageAgents(h))
