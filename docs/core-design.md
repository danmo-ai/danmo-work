...
- 产品内置连接器（如 `danmo-make`、`codegraph`、`github`）由 bootstrap `ensureBuiltinConnectors` 以固定 server id 自动 seed；专家/技能走 embedded `prompt/agents` + `prompt/skills`（UI「内置」= 存在 template）。`codegraph` 为 stdio + 绑定专家，CLI 为 [sunerpy/codegraph-rust](https://github.com/sunerpy/codegraph-rust)（`scripts/fetch_codegraph.sh` 拉压缩包）；按平台打包的 `scripts/first_launch/{darwin,linux,windows}/post-install.*` 在后端启动后由 `StartFirstLaunchAsync` 异步执行（解压 CodeGraph 等），完成后刷新 builtin connectors。首次 `delegate_agent`→`codegraph` 时异步 `codegraph init`，完成前专家降级 `read_file`/`grep`。`github` 专家包 = 技能 + **绑定型**官方远程 MCP（server id `github`，`AmbientMount=false`，市场不再提供 `github-mcp`）+ 降级链 **MCP → `gh` → `git`**：`GitHubMCPReady` 为真用 `mcp_github_*`，否则 `gh`（`ResolveGhBin`），再否则 `git`（`ResolveGitBin`，仅 remote/push/fetch；Issue/PR 仍报阻塞）；首次委派注入 `[github-access: mcp|gh|git|none]`。市场目录会过滤 `BuiltinConnectorIDs` 与遗留 `github-mcp`，并拒绝安装。
