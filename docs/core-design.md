...
- 产品内置连接器（如 `danmo-make`、`codegraph`、`github`）由 bootstrap `ensureBuiltinConnectors` 按固定 server id 自动写入；专家与技能来自 embedded `prompt/agents`、`prompt/skills`（界面「内置」= 存在模板）。`codegraph`：stdio + 绑定专家；首次 `delegate_agent`→`codegraph` 时异步 `codegraph init`，完成前可退回 `read_file`/`grep`。`github`：绑定型官方远程 MCP（id=`github`，不 ambient；市场不再提供 `github-mcp`）+ 本机能力依次尝试 MCP → `gh` → `git`（`git` 仅限 remote/push/fetch；Issue/PR 仍需 MCP 或 `gh`）；委派时注入 `[github-access: mcp|gh|git|none]`。市场列表会藏掉 `BuiltinConnectorIDs` 与遗留 `github-mcp`，并拒绝安装。
