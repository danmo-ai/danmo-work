...
- **Office co-edit**: [`docs/demo/office-coedit-tour.html`](../demo/office-coedit-tour.html)?lang=zh&tour=1  
...
Office co-edit tour covers: Intent → Propose → Review → Commit · Doc / Slides / Sheet · AI Diff Keep/Revert/hunks.

Post tip: attach `product-tour-zh.mp4` (or GIF) above the repo link; for Office-focused posts, link `office-coedit-tour.html`.
...
**EN (≤350 chars):**
```text
Open-source AI Work Agent — coding-agent-grade loop for long-horizon work: human↔AI Office co-edit (docs/slides/sheets), MCP & IM on one trail. Self-hosted, MIT.
```
...
同一思维链多 Agent；一切皆工具（含 `ask_user`）；Turn Log 可恢复可回放。Document Stage 上**人与 AI 共审编辑**文档/幻灯片/表格（提案 → Diff → 保留/回滚）；微信·飞书·企微·QQ 同一套 Loop，工具在本机跑。

MIT，可自托管。macOS：`brew tap danmo-ai/tap && brew install --cask danmo-work`，或 https://github.com/danmo-ai/danmo-work/releases/latest

Office 共审动画：https://github.com/danmo-ai/danmo-work/blob/main/docs/demo/office-coedit-tour.html

---

## V2EX / 掘金 / 少数派（中）

**标题：** 开源 AI Work Agent：一流 Coding 底盘，更大的是长程工作与人机共审 Office

主流开源 Agent 多半停在终端写代码。Danmo Work 保留同级执行 Loop，面向**长程工作产物**——代码、调研、文档、幻灯片、连接器、IM。

- **纯 LLM 驱动**：无 Graph / Mode；`delegate_agent` 同一思维链硬隔离
- **人 ↔ AI Office 共审**：Document Stage 上 Intent → Propose → Review → Commit；AI Diff 保留/回滚/分块接受；文本 SoT
...
共审动画：https://github.com/danmo-ai/danmo-work/blob/main/docs/demo/office-coedit-tour.html  
Repo：https://github.com/danmo-ai/danmo-work
...
**Danmo Work** — open-source AI *Work* Agent. Coding-agent-grade loop, wider job: human↔AI Office co-edit (docs/slides/sheets) + MCP + IM on one trail.

Pure LLM-driven. Sub-agents & humans are Tools. Turn Log → resume/replay. AI Diff Keep/Revert on Document Stage. Self-hosted, MIT.
