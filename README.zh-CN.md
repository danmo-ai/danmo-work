...
| **飞书** | 出站 WebSocket（无需公网 URL） | 开放平台 → 长连接收事件/卡片回调 → 设置 → 飞书；支持交互卡片审批、进度更新、`/project` 切换 |
| **企业微信** | 出站 WebSocket（`openws.work.weixin.qq.com`） | 管理后台 → 智能机器人长连接 → 设置 → 企业微信（Bot ID / Secret） |
| **QQ** | 出站 Gateway WebSocket（无需公网 URL） | q.qq.com 创建机器人 → 设置 → QQ（App ID / Secret）；键盘审批、C2C 流式、`/project` 切换 |
