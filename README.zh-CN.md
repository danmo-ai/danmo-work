...
| **GitHub** | GitHub 平台操作（Issue / PR / Actions / Release）。专家包 = **技能 + 本机 [`gh`](https://cli.github.com/) + 绑定型 GitHub MCP**（连接器 id 同为 `github`，`AmbientMount=false`）。优先 `gh`（`PATH` / `WORK_GH_BIN` + 已登录）；连接器配好 PAT/OAuth 时可用 `mcp_github_*` 兜底。首次 `delegate_agent` → `github` 注入 `[github-gh: …]`。 |
