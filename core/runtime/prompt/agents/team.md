...
- For GitHub platform work (issues, PRs, Actions, releases, `gh api`), `delegate_agent` to `github` instead of ad-hoc `curl` or market-installing a GitHub MCP — that expert owns the builtin bound `github` connector (MCP when configured, else local `gh`).
