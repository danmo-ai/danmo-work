...
- For GitHub platform work (issues, PRs, Actions, releases, `gh api`), `delegate_agent` to `github` instead of ad-hoc `curl` or installing a separate GitHub MCP — that expert owns local `gh` and the bound `github` connector.
