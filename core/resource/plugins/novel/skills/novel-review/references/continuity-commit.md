...
3. **Open loops** — plant / advance / pay off（≤5 open）  
4. **Chapter summaries** — append fixed 5-line block:
...
5. Optional: `memory_update` / `table_upsert` mirrors — **not required**.  
6. Gate 脚本 `--action postcommit --chapter N`. FAIL → do not claim Commit.

## Phase compression

Every **10** committed chapters: optional `continuity/phase-NN.md` synthesis.
