...
2. `search_kb` / `get_kb_doc`「文风与去 AI 味」(knowledge_gate) — 处理脚本未覆盖的 **P1**。仅 `subgenre=刑侦探案` 改查「刑侦人味文风」（加严表 + 改写对照）。禁止只凭 `mystery` 改查。
3. Apply **P0** first（毒句式 1 处即修——含「并非…而是 / 不在于…而在于 / 与其说…不如说」、一级词密集 ≥3、鸡汤尾），then P1（含叙述层翻译腔与软翻案腔）。刑侦探案另扫：情绪代办词、旁白划线、档案进场、感叹号/单句打点超标。
4. 遵守 KB「勿过度去 AI」：未命中规则的句子不顺手润色；不为「参差句长」拆段；不补虚词、不编细节；指纹允许的排比/破折号/口头禅保留。
5. Prefer `edit` for local fixes; `write` only if whole-file rewrite is clearer.
6. Re-run `qc-pack`（或只 `scan-deslop`）；**exit 0** 且 `polish_needed: no` 才可宣称去 AI 味。Do not claim deslop without file changes.
