...
剧情单元用 **单元卡**（模板 `volume-outline.md`）：必填单元ID、章范围、**单元节拍（章功能分配）**、单元功能、主角局部目标、因果入口、核心阻碍、关键选择、主爽点形态、兑现归属、禁止提前释放、下一单元钩子、终局边界。节拍须覆盖该单元章范围（建立期待→尝试→加压→决断→兑现→余波，可按题材删并）。缺卡、关键字段空、或节拍未覆盖章范围 → **不要进入章纲**：章纲的 `unit_id` / `purpose` / `beats` / `pleasure_point` 必须能指回某个单元卡 + 锚点。
