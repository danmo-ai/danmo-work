...
1. **总纲** — 一句话故事, 读者承诺, 分卷结构表, 主线伏笔, 结局方向
2. **卷纲** — 卷目标, 冲突与起终, 节奏锚点, **剧情单元（一段章）**, 情绪/人物弧, 反转, 伏笔
3. **章合同** — 下一技能 `novel-write`（`chapter-contract.md`）；YAML under `chapters/`

卷纲写到「一段章」的剧情单元为止。禁止在 `outline/` 写单章任务/爽点/钩子。

没有填满的剧情单元表，**不要进入章合同**：合同的 `purpose` / `beats` / `pleasure_point` 必须能指回某个单元 + 锚点。
...
- `outline/volumes/vXX.md` — from `volume-outline.md` template
