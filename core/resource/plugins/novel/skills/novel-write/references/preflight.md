...
6. 本章 `beats` / `state_deltas` 点名的 `canon/cast/*.md`（asset_gate）。未上场角色不读。人物卡只取公开段 + 三锚点；隐藏真相应在 author-lore。
7. `search_kb` 本阶段 **1–2** 个主题（knowledge_gate）。写钩子/接钩用「爽点与追读」。**ch001–ch003** 加「节奏与结构」；写正文前 `read_skill` `opening-chapters.md`。悬疑/反转题再查「连载三轨」。
...
```text
[YYYY-MM-DD chNNN] state:writing | contract:accepted | public-lore | tracking | cast:3 | foreshadows:open:2 | KB:爽点与追读 | gate:PASS
```
...
Agent 完成 preflight + gate 脚本后更新（供工作台展示）：

```yaml
gates:
  knowledge: pass|fail|unknown
  asset: pass|fail|unknown
  qc: unknown          # 写前不填 qc；precommit 后再写
blockers: []  # 人类可读阻断原因；可抄 gate 脚本 BLOCKING
```

Gate 脚本 FAIL → `gates.asset` 或对应项 `fail`，`blockers` 列出缺失项。

## 阻断

Gate 脚本 preflight FAIL、缺少必读文件、合同非 `accepted`（写正文时）、`unit_id` 空或对不上卷纲单元、Frozen_Canon 空（续写）、批次未冻结（批量写）→ **停止**，列缺失项。空/错 `unit_id` 退回 `novel-plan` 补卷纲，不要空造合同。
